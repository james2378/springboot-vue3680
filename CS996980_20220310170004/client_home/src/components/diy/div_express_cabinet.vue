<template>
	<div class="diy_details_box diy_div_express_cabinet">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "express_cabinet_id",
					source_id: this.obj["express_cabinet_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_cabinet/set?express_cabinet_id=' + _this.obj["express_cabinet_id"], {
								praise_len
							}, (res) => {
								if(res.result){
