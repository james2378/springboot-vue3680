<template>
	<div class="diy_details_box diy_div_apply_for_receipt">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "apply_for_receipt_id",
					source_id: this.obj["apply_for_receipt_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/apply_for_receipt/set?apply_for_receipt_id=' + _this.obj["apply_for_receipt_id"], {
								praise_len
							}, (res) => {
								if(res.result){
