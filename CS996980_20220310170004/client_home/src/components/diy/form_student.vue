<template>
	<div>
		<b-form-group v-if="$check_register_field('add','gender','/student/view')" id="input-group-0" label="性别" label-for="input-0">
			<b-form-select id="input-0" v-model="form['gender']" :options="getDropList(list_gender)"></b-form-select>
		</b-form-group>
		<b-form-group v-if="$check_register_field('add','student_number','/student/view')" id="input-group-1" label="学号" label-for="input-1">
			<b-form-input id="input-1" v-model="form['student_number']" type="text" placeholder="学号" trim></b-form-input>
		</b-form-group>
	</div>
</template>
