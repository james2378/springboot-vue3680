<template>
	<div class="diy_details_box diy_div_courier_services_company">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "courier_services_company_id",
					source_id: this.obj["courier_services_company_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/courier_services_company/set?courier_services_company_id=' + _this.obj["courier_services_company_id"], {
								praise_len
							}, (res) => {
								if(res.result){
