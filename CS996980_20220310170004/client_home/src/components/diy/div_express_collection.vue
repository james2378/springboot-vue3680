<template>
	<div class="diy_details_box diy_div_express_collection">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "express_collection",
					source_field: "express_collection_id",
					source_id: this.obj["express_collection_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_collection/set?express_collection_id=' + _this.obj["express_collection_id"], {
								praise_len
							}, (res) => {
								if(res.result){
