<template>
	<div class="diy_details_box diy_div_express_delivery">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "express_delivery_id",
					source_id: this.obj["express_delivery_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_delivery/set?express_delivery_id=' + _this.obj["express_delivery_id"], {
								praise_len
							}, (res) => {
								if(res.result){
