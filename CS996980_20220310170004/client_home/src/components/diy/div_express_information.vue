<template>
	<div class="diy_details_box diy_div_express_information">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "express_information_id",
					source_id: this.obj["express_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_information/set?express_information_id=' + _this.obj["express_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
