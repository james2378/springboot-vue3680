<template>
	<div class="diy_details_box diy_div_use_express_delivery">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "use_express_delivery_id",
					source_id: this.obj["use_express_delivery_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/use_express_delivery/set?use_express_delivery_id=' + _this.obj["use_express_delivery_id"], {
								praise_len
							}, (res) => {
								if(res.result){
