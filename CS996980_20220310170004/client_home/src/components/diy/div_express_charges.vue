<template>
	<div class="diy_details_box diy_div_express_charges">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "express_charges_id",
					source_id: this.obj["express_charges_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_charges/set?express_charges_id=' + _this.obj["express_charges_id"], {
								praise_len
							}, (res) => {
								if(res.result){
