<template>
	<div class="diy_details_box diy_div_express_pick_up">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "express_pick_up_id",
					source_id: this.obj["express_pick_up_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_pick_up/set?express_pick_up_id=' + _this.obj["express_pick_up_id"], {
								praise_len
							}, (res) => {
								if(res.result){
