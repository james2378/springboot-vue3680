<template>
	<div class="diy_details_box diy_div_express_logistics">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "express_logistics_id",
					source_id: this.obj["express_logistics_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/express_logistics/set?express_logistics_id=' + _this.obj["express_logistics_id"], {
								praise_len
							}, (res) => {
								if(res.result){
