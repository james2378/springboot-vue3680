<template>
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','corporate_name') || $check_field('add','corporate_name') || $check_field('set','corporate_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="公司名称" prop="corporate_name">
					<el-input id="corporate_name" v-model="form_sub['corporate_name']" placeholder="请输入公司名称"
							  v-if="user_group === '管理员' || (form_sub['courier_services_company_id'] && $check_field('set','corporate_name')) || (!form_sub['courier_services_company_id'] && $check_field('add','corporate_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','person_in_charge') || $check_field('add','person_in_charge') || $check_field('set','person_in_charge')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="负责人" prop="person_in_charge">
					<el-input id="person_in_charge" v-model="form_sub['person_in_charge']" placeholder="请输入负责人"
							  v-if="user_group === '管理员' || (form_sub['courier_services_company_id'] && $check_field('set','person_in_charge')) || (!form_sub['courier_services_company_id'] && $check_field('add','person_in_charge'))"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				group_table: "courier_services_company",
				is_password: true,
				...
				form_sub: {
				    ...
				    "user_id": 0,
					"courier_services_company_id": 0 // ID
				},
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["courier_services_company_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?courier_services_company_id=" + form_sub["courier_services_company_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/courier_services_company/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/courier_services_company/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/courier_services_company/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/courier_services_company/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/courier_services_company/view','get');
